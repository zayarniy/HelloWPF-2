...
namespace FunWithCSharpAsync
{
    ...
    public partial class MainWindow : Window
    {
        ...
        {            
            ...
            /*
            Ключевое слово await отвечает за извлечение внутреннего возвращаемого значения, содержащегося в объекте Task.
            Из-за отсутствия этого ключевого слова возникает несовпадение типов. 
            */
        ...
        private void btnButton_Click3(object sender, RoutedEventArgs e)
        {
...
/*
Первым делом, обратите внимание, что обработчик события Click кнопки помечен ключевым словом async. Это значит, что данный метод должен вызываться в неблокирующей манере.
Кроме того, в реализации обработчика события перед именем вызываемого метода используется ключевое слово await. Это важный момент: если метод декорируется ключевым словом 
async, но не имеет хотя бы одного внутреннего вызова метода с применением await, получается блокирующий, синхронный вызов (на самом деле в таком случае компилятор выдаст соответствующее предупреждение).
Мы должны были воспользоваться классом Task из пространства имен System. Threading .Tasks для проведения рефакторинга метода DoWork () с целью обеспечения его корректного функционирования. 
В сущности, вместо возврата специфического значения напрямую (объекта string в текущем примере) мы возвращаем объект Task<T>, где обобщенный параметр типа Т — это лежащее в основе возвращаемое значение.
...
Когда поток выполнения достигает await, вызывающий поток приостанавливается до тех пор, пока вызов не будет завершен. Запустив эту версию приложения, вы обнаружите, что можно щелкнуть на кнопке и сразу же вводить в текстовом поле. 
Спустя 10 секунд заголовок окна будет обновлен сообщением о завершении работы.
*/
